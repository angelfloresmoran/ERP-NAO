...
<ul>
  ...
  <main>
    ...
    <h1>Pagos del Proyecto</h1> <br>
        <h4> <a href="formGastosProyecto.php" class="a-forms"> <img src="./assets/bx-add-to-queue.svg">Gastos del Proyecto</a></h4>
        <div> <br>
        <label>Filtrar por Fecha:</label>
        <input type="date" id="filtroFecha">               
        <input type="text" id="busqueda" placeholder="Buscar por nombre...">
        
        <?php $proyecto = mysqli_query($conex, "SELECT id, nombre FROM proyectos;"); ?>
        
        <label>Filtrar por Proyecto:</label>
        <select id="proyectos">
        <option value="">Mostrar todos</option>
        <?php while ($proy = mysqli_fetch_array($proyecto)) { ?>
        <option value="<?php echo $proy['nombre']?>"><?php echo $proy['nombre']?></option>
        <?php } ?>
        </select> 
        </div>  
        <div class="tabla">
    <div class="tabla-contenido">
        <table id="tablax" class="table table-striped table-bordered" style="width:100%">
            ...
            <?php
                ...
                require_once "crud/conex.php";
                $consulta = mysqli_query($conex, "SELECT g.id, g.nombre AS nombre_gasto, g.descripcion, g.fecha, g.archivo, g.proyecto, p.nombre AS nombre_proy FROM gastosproy g JOIN proyectos p ON g.proyecto = p.id ORDER BY g.fecha DESC;" );
            ...
            <tr>
                <td><?php echo $fila['nombre_gasto'] ?></td>
                <td><?php echo $fila['nombre_proy'] ?></td>
                <td style="width:90px;"><?php echo $fila['fecha'] ?></td>
                <td><?php echo $fila['descripcion'] ?></td>
                <td><?php echo $fila['archivo'] ?></td>
                
                <td><a href="crud/dArchivosGastos.php?id= <?php echo $fila['id'] ;?>" class="btn btn-primary"><img src="./assets/bx-cloud-download.svg"></a>
                </td>
                <td>
                <a href="editarPago.php?id=<?php echo $fila['id']; ?>" class="btn btn-warning"><img src="./assets/bx-edit.svg"></a>
    ...
    <script src="js/filtrarReportes.js"></script>
    <script>
    function confirmarEliminacion(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "crud/eliminarPago.php?id=" + id;
            }
        });
    }
</script>
